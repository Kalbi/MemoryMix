...
if(ds_list_size(global.p1unused_actions) == 0) {
	global.double_cue = true;
}
if (global.double_cue == false) {	
		//store new action in list
		ds_list_add(global.p1round_actions, ds_list_find_value(global.p1unused_actions, 0));

		//remove used action from unused actions list
		ds_list_delete(global.p1unused_actions, 0);
}else{
	
	ds_list_add(global.p1round_actions, ds_list_find_value(global.p1unused_actions2, 0));
	
	ds_list_delete(global.p1unused_actions2, 0);
}

if(ds_list_size(global.p1unused_actions2) == 0) {
	beatgame = true;
}
